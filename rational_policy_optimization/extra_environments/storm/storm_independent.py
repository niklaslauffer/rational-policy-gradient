...
class InTheGrid_2p_independent(MultiAgentEnv):
    """
    ...
    def __init__(
        ...
        def _get_obs(state: State) -> jnp.ndarray:
            ...
            return {
                "agent_0": jnp.concatenate(
                    [obs1.ravel(), inv1.ravel()], dtype=jnp.uint8
                ),
                "agent_1": jnp.concatenate(
                    [obs2.ravel(), inv2.ravel()], dtype=jnp.uint8
                ),
            }
        ...
        def _get_reward(state: State) -> jnp.ndarray:
            ...
            # make inv uniform if empty
            r1 = inv1 @ payoff_matrix[0] @ inv2.T
            r2 = inv1 @ payoff_matrix[1] @ inv2.T
            r1 = jnp.nan_to_num(r1, nan=0.0)
            r2 = jnp.nan_to_num(r2, nan=0.0)
        ...
        def _interact(
            ...
            interact = state.inner_t == (self.num_inner_steps - 2)
        ...
        def _step(key: chex.PRNGKey, state: State, actions: Tuple[int, int]):
            ...
            # jax.debug.print("red_reward {x}", x=red_reward)
            # jax.debug.print("blue_reward {x}", x=blue_reward)
    ...
    def render_agent_view(self, state: State, agent: int) -> Tuple[onp.ndarray]:
        ...
        for j in range(0, grid.shape[1]):
            for i in range(0, grid.shape[0]):
                ...
                tile_img = InTheGrid_2p_independent.render_tile(
                    cell,
                    agent_dir=agent_dir,
                    agent_hat=agent_hat,
                    highlight=None,
    ...
    def render(self, state: State) -> onp.ndarray:
        ...
        grid[(state.red_pos[0], state.red_pos[1])] = jnp.int8(Items.red_agent)
        grid[(state.blue_pos[0], state.blue_pos[1])] = jnp.int8(Items.blue_agent)
