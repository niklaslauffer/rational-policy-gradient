...
class InTheGrid_2p_sabotage(MultiAgentEnv):
    """
    ...
    def __init__(
        ...
        _shape = (OBS_SIZE, OBS_SIZE, len(Items) - 1 + 4) if self.cnn else (OBS_SIZE**2 * (len(Items) - 1 + 4),)
        obs_space = spaces.Box(
            low=0,
            high=NUM_COINS,
            shape=jnp.prod(jnp.array(_shape)) + NUM_COIN_TYPES + 4,
            dtype=jnp.uint8,
        )
        ...
        def _get_obs(state: State) -> jnp.ndarray:
            ...
            x, y = _get_obs_point(state.blue_pos[0], state.blue_pos[1], state.blue_pos[2])
            ...
            # jax.debug.print("_grid2")
            # jax.debug.print("{x}", x=jnp.moveaxis(_grid2, 2, 0))
        ...
        def _interact(state: State, actions: Tuple[int, int]) -> Tuple[bool, float, float, State]:
            # if interact
            a0, a1 = actions
            ...
            state = state.replace(grid=jnp.where(state.grid == interact_idx, jnp.int8(Items.empty), state.grid))

            # check 1 ahead
            red_target = jnp.clip(state.red_pos + STEP[state.red_pos[2]], 0, GRID_SIZE - 1)
            blue_target = jnp.clip(state.blue_pos + STEP[state.blue_pos[2]], 0, GRID_SIZE - 1)

            red_interact = state.grid[red_target[0], red_target[1]] == Items.blue_agent
            blue_interact = state.grid[blue_target[0], blue_target[1]] == Items.red_agent

            # check 2 ahead
            red_target_ahead = jnp.clip(state.red_pos + 2 * STEP[state.red_pos[2]], 0, GRID_SIZE - 1)
            blue_target_ahead = jnp.clip(state.blue_pos + 2 * STEP[state.blue_pos[2]], 0, GRID_SIZE - 1)

            red_interact_ahead = state.grid[red_target_ahead[0], red_target_ahead[1]] == Items.blue_agent
            blue_interact_ahead = state.grid[blue_target_ahead[0], blue_target_ahead[1]] == Items.red_agent

            # check to your right  - clip can't be used here as it will wrap down
            red_target_right = state.red_pos + STEP[state.red_pos[2]] + STEP[(state.red_pos[2] + 1) % 4]
            ...
            blue_target_right = state.blue_pos + STEP[state.blue_pos[2]] + STEP[(state.blue_pos[2] + 1) % 4]
            ...
            red_interact_right = state.grid[red_target_right[0], red_target_right[1]] == Items.blue_agent
            blue_interact_right = state.grid[blue_target_right[0], blue_target_right[1]] == Items.red_agent

            # check to your left
            red_target_left = state.red_pos + STEP[state.red_pos[2]] + STEP[(state.red_pos[2] - 1) % 4]
            oob_red = jnp.logical_or(
            ...
            blue_target_left = state.blue_pos + STEP[state.blue_pos[2]] + STEP[(state.blue_pos[2] - 1) % 4]
            oob_blue = jnp.logical_or(
            ...
            red_interact_left = state.grid[red_target_left[0], red_target_left[1]] == Items.blue_agent
            blue_interact_left = state.grid[blue_target_left[0], blue_target_left[1]] == Items.red_agent
            ...
            interact = jnp.logical_and(interact, jnp.logical_and(red_pickup, blue_pickup))
            ...
            red_sabotage = jnp.logical_and(state.red_pos[0] == 0, state.red_pos[1] == 0)
            blue_sabotate = jnp.logical_and(state.blue_pos[0] == 0, state.blue_pos[1] == 0)
            sabotage = jnp.logical_or(red_sabotage, blue_sabotate)

            red_reward = jnp.where(sabotage, -0.1, red_reward)
            blue_reward = jnp.where(sabotage, -0.1, blue_reward)
        ...
        def _step(key: chex.PRNGKey, state: State, actions: Tuple[int, int]):
            ...
            # turning red
            new_red_pos = jnp.int8((state.red_pos + ROTATIONS[action_0]) % jnp.array([GRID_SIZE + 1, GRID_SIZE + 1, 4]))

            # moving red
            red_move = action_0 == Actions.forward
            new_red_pos = jnp.where(red_move, new_red_pos + STEP[state.red_pos[2]], new_red_pos)
            ...
            new_blue_pos = jnp.int8(
                (state.blue_pos + ROTATIONS[action_1]) % jnp.array([GRID_SIZE + 1, GRID_SIZE + 1, 4], dtype=jnp.int8)
            )

            # moving blue
            blue_move = action_1 == Actions.forward
            new_blue_pos = jnp.where(blue_move, new_blue_pos + STEP[state.blue_pos[2]], new_blue_pos)
            ...
            new_blue_pos = jnp.where(
                collision * blue_move * red_move * (red_takes_square),  # if both collide and blue doesn't take square
                state.blue_pos,
                new_blue_pos,
            )

            # update inventories
            red_red_matches = state.grid[new_red_pos[0], new_red_pos[1]] == Items.red_coin
            red_blue_matches = state.grid[new_red_pos[0], new_red_pos[1]] == Items.blue_coin
            blue_red_matches = state.grid[new_blue_pos[0], new_blue_pos[1]] == Items.red_coin
            blue_blue_matches = state.grid[new_blue_pos[0], new_blue_pos[1]] == Items.blue_coin

            state = state.replace(red_inventory=state.red_inventory + jnp.array([red_red_matches, red_blue_matches]))
            state = state.replace(
                blue_inventory=state.blue_inventory + jnp.array([blue_red_matches, blue_blue_matches])
            )

            # update grid
            state = state.replace(grid=state.grid.at[(state.red_pos[0], state.red_pos[1])].set(jnp.int8(Items.empty)))

            state = state.replace(grid=state.grid.at[(state.blue_pos[0], state.blue_pos[1])].set(jnp.int8(Items.empty)))

            state = state.replace(grid=state.grid.at[(new_red_pos[0], new_red_pos[1])].set(jnp.int8(Items.red_agent)))

            state = state.replace(
                grid=state.grid.at[(new_blue_pos[0], new_blue_pos[1])].set(jnp.int8(Items.blue_agent))
            )
            ...
            state = state.replace(freeze=jnp.where(interact, freeze_penalty, state.freeze))

            # if we didn't interact, then we decrement freeze
            state = state.replace(freeze=jnp.where(state.freeze > 0, state.freeze - 1, state.freeze))
        ...
        def _soft_reset_state(key: jnp.ndarray, state: State) -> State:
            ...
            for i in range(NUM_COINS):
                grid = grid.at[state.red_coins[i, 0], state.red_coins[i, 1]].set(jnp.int8(Items.red_coin))

            for i in range(NUM_COINS):
                grid = grid.at[state.blue_coins[i, 0], state.blue_coins[i, 1]].set(jnp.int8(Items.blue_coin))

            agent_pos = jax.random.choice(key, AGENT_SPAWNS, shape=(), replace=False)

            player_dir = jax.random.randint(key, shape=(2,), minval=0, maxval=3, dtype=jnp.int8)
            # player_dir = jnp.array([0, 0], dtype=jnp.int8)
        ...
        def _reset_state(key: jnp.ndarray) -> Tuple[jnp.ndarray, State]:
            ...
            player_dir = jax.random.randint(subkey, shape=(2,), minval=0, maxval=3, dtype=jnp.int8)
            # player_dir = jnp.array([0, 0], dtype=jnp.int8)
            player_pos = jnp.array([agent_pos[:2, 0], agent_pos[:2, 1], player_dir]).T
            grid = jnp.zeros((GRID_SIZE, GRID_SIZE), jnp.int8)
            grid = grid.at[player_pos[0, 0], player_pos[0, 1]].set(jnp.int8(Items.red_agent))
            grid = grid.at[player_pos[1, 0], player_pos[1, 1]].set(jnp.int8(Items.blue_agent))
            ...
            for i in range(NUM_COINS):
                grid = grid.at[red_coins[i, 0], red_coins[i, 1]].set(jnp.int8(Items.red_coin))

            for i in range(NUM_COINS):
                grid = grid.at[blue_coins[i, 0], blue_coins[i, 1]].set(jnp.int8(Items.blue_coin))
    ...
    def state_space(self) -> spaces.Dict:
        ...
        _shape = (GRID_SIZE, GRID_SIZE, NUM_TYPES + 4) if self.cnn else (GRID_SIZE**2 * (NUM_TYPES + 4),)
    ...
    def render_tile(
        ...
        if agent_dir is not None:
            ...
            # Rotate the agent based on its direction
            tri_fn = rotate_fn(tri_fn, cx=0.5, cy=0.5, theta=0.5 * math.pi * (1 - agent_dir))
    ...
    def render_agent_view(self, state: State, agent: int) -> Tuple[onp.ndarray]:
        ...
        for j in range(0, grid.shape[1]):
            for i in range(0, grid.shape[0]):
                ...
                tile_img = InTheGrid_2p_sabotage.render_tile(
                    cell,
                    agent_dir=agent_dir,
                    agent_hat=agent_hat,
                    highlight=None,
    ...
    def render(self, state: State) -> onp.ndarray:
        ...
        grid = onp.pad(grid, ((PADDING, PADDING), (PADDING, PADDING)), constant_values=5)

        startx, starty = self.get_obs_point(state.red_pos[0], state.red_pos[1], state.red_pos[2])
        highlight_mask[startx : startx + OBS_SIZE, starty : starty + OBS_SIZE] = True

        startx, starty = self.get_obs_point(state.blue_pos[0], state.blue_pos[1], state.blue_pos[2])
        highlight_mask[startx : startx + OBS_SIZE, starty : starty + OBS_SIZE] = True
        if state.freeze > 0:
            ...
            if r1 == -r2:
                ...
                if r1 > r2:
                    ...
                    img[:, : width_px // 2, :] = onp.tile(PLAYER2_COLOUR, (img.shape[0], img.shape[1] // 2, 1))
                    img[:, width_px // 2 :, :] = onp.tile(PLAYER1_COLOUR, (img.shape[0], img.shape[1] // 2, 1))
            else:
                # otherwise we got some cool general sum game
                welfare = r1 + r2
                if welfare > 5:
                    # cooperate
                    img[:, width_px // 2 :, :] = onp.tile(GREEN_COLOUR, (img.shape[0], img.shape[1] // 2, 1))
                else:
                    img[:, width_px // 2 :, :] = onp.tile(RED_COLOUR, (img.shape[0], img.shape[1] // 2, 1))
                if r1 > r2:
                    # red won
                    img[:, : width_px // 2, :] = onp.tile(PLAYER1_COLOUR, (img.shape[0], img.shape[1] // 2, 1))
                elif r1 < r2:
            ...
            for j in range(0, grid.shape[1]):
                for i in range(0, grid.shape[0]):
                    ...
                    agent_dir = state.blue_pos[2].item() if blue_agent_here else agent_dir
                    agent_hat = False
                    agent_hat = bool(state.red_inventory.sum() > INTERACT_THRESHOLD) if red_agent_here else agent_hat
                    agent_hat = bool(state.blue_inventory.sum() > INTERACT_THRESHOLD) if blue_agent_here else agent_hat
        ...
        for j in range(0, NUM_COIN_TYPES):
            ...
            for i in range(int(num_coins)):
                ...
                if j == 0:
                    ...
                    cell = 100
                tile_img = InTheGrid_2p_sabotage.render_tile(cell, tile_size=tile_height)
                ymin = j * tile_height
                ymax = (j + 1) * tile_height
                xmin = i * tile_width
                xmax = (i + 1) * tile_width
                img[ymin:ymax, xmin:xmax, :] = onp.resize(tile_img, (tile_height, tile_width, 3))
        return img
...
AGENT_SPAWNS_SIMPLE = jnp.array(
    [[(j, i), (GRID_SIZE - 1 - j, GRID_SIZE - 1 - i)] for (i, j) in AGENT_SPAWNS_SIMPLE],
    dtype=jnp.int8,
).reshape(-1, 2, 2)
